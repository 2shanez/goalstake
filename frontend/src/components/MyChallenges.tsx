...
export function MyChallenges() {
  ...
  if (!address) {
    return (
      <div className="bg-[#0d0d0d] border border-[#222] rounded-2xl p-8 text-center">
        <p className="text-[#888]">Connect wallet to view challenges</p>
      </div>
    )
  }
  ...
  if (!challengeIds || challengeIds.length === 0) {
    return (
      <div className="bg-[#0d0d0d] border border-[#222] rounded-2xl p-8">
        <div className="text-center mb-6">
          <div className="w-12 h-12 rounded-full bg-[#1a1a1a] flex items-center justify-center mx-auto mb-4">
            <span className="text-2xl">🎯</span>
          </div>
          <p className="text-[#888]">No challenges yet</p>
          <p className="text-sm text-[#555] mt-1">Create your first challenge to get started</p>
        </div>
        
        {loserPool !== undefined && loserPool > 0n && (
          <div className="p-4 rounded-xl bg-[#2EE59D]/5 border border-[#2EE59D]/20">
            <p className="text-xs text-[#888] text-center">Current Prize Pool</p>
  ...
  return (
    <div className="space-y-4">
      {/* Loser Pool Banner */}
      {loserPool !== undefined && loserPool > 0n && (
        <div className="bg-[#0d0d0d] border border-[#2EE59D]/20 rounded-xl p-4">
          <div className="flex items-center justify-between">
            <span className="text-sm text-[#888]">Prize Pool</span>
...
function ChallengeCardWrapper({ 
  ...
  if (isLoading) {
    return (
      <div className="bg-[#0d0d0d] border border-[#222] rounded-xl p-6 animate-pulse">
        <div className="h-5 bg-[#1a1a1a] rounded w-1/3 mb-3"></div>
...
function ChallengeCard({ challenge }: { challenge: Challenge }) {
  ...
  return (
    <div className={`bg-[#0d0d0d] border rounded-xl p-5 ${
      challenge.settled 
        ? challenge.success 
          ? 'border-[#2EE59D]/50' 
          : 'border-red-500/50'
